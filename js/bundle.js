...
function calc() {
  ...
  function getStaticInformation(selector, activeClass) {
    var elements = document.querySelectorAll(selector); // При замене parentSelector на selector, нужно учитывать это здесь
...
function cards() {
  ...
  var MenuCard = /*#__PURE__*/function () {
    ...
    function MenuCard(src, alt, title, descr, price, parentSelector) {
      ...
      this.title = title;
      this.descr = descr;
      this.price = price; // Но [] в логическом контексте true
    ...
    _createClass(MenuCard, [{
      ...
      value: function render() {
        var element = document.createElement('div'); // С урока
  ...
  axios.get('http://localhost:3000/menu').then(function (cards) {
    cards.data.forEach(function (obj) {
      _construct(MenuCard, _toConsumableArray(Object.values(obj)).concat(['.menu .container'])).render();
    });
...
function forms(modalTimerId) {
  // Forms
  var forms = document.forms; // Для блока после формы - состояние запроса для пользователя
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (event) {
      event.preventDefault(); // const statusMessage = document.createElement('div');

      var statusMessage = document.createElement('img');
      statusMessage.src = message.loading; // Уст. по центру (лучше через css классы)
      ...
      var formData = new FormData(form); // Но, напр., нам нужно передать в формате JSON
      ...
      var json = JSON.stringify(Object.fromEntries(formData.entries())); // Отправим
      ...
      (0,_services_services__WEBPACK_IMPORTED_MODULE_7__.postData)('http://localhost:3000/requests', json).then(function (data) {
        console.log(data);
  ...
  function showThanksModal(message) {
    var prevModalDialog = document.querySelector('.modal__dialog'); // Нужно скрывать, а не удалять, чтобы в дальнейшем пользоваться изначальным функционалом

    prevModalDialog.classList.add('hide'); // Заново открываем

    (0,_modal__WEBPACK_IMPORTED_MODULE_6__.openModal)('.modal', modalTimerId); // Формируем новое modal__dialog

    var thanksModal = document.createElement('div');
    thanksModal.classList.add('modal__dialog'); // Чтоб крестик работал, нужно применить делегирование к modal

    thanksModal.innerHTML = "\n      <div class=\"modal__content\">\n        <div class=\"modal__close\" data-close>&times;</div>\n        <div class=\"modal__title\">".concat(message, "</div>\n      </div>\n    "); // Помещаем в div.modal
    // modal.append(thanksModal);

    document.querySelector('.modal').append(thanksModal); // Через время новое мод. окно должно пропасть и появиться старое
...
function slider(_ref) {
  ...
  var slideIndex = 1,
      offset = 0; // Функция удаления не чисел с исп. регул. выр.
  ...
  function setCurrentCount(ind, current) {
    if (ind < 10) {
      current.textContent = "0".concat(ind);
    } else {
  ...
  setTotalCount(slides, total);
  setCurrentCount(slideIndex, current); // if (slides.length < 10) {
  //   total.textContent = `0${slides.length}`;
  // } else {
  //   total.textContent = slides.length;
  // }
  ...
  next.addEventListener('click', function () {
    ...
    slidesField.style.transform = "translate(-".concat(offset, "px)"); // Установка номера показываемого слайдера
    ...
    setCurrentCount(slideIndex, current); // if (slideIndex < 10) {
    //   current.textContent = `0${slideIndex}`;
    // } else {
    //   current.textContent = slideIndex;
    // }
    ...
    setDotsOpacity(slideIndex, dots); // dots.forEach(dot => dot.style.opacity = 0.5);
...
function timer(selector, deadline) {
  ...
  function setTimer(selector, endtime) {
    ...
    function updateTimer() {
      // Получили объект
      var t = getTimeRemaining(endtime); // Если дедлайн прошел
...
/* harmony import */ var dom_node_polyfills__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! dom-node-polyfills */ "./node_modules/dom-node-polyfills/index.js");
/* harmony import */ var dom_node_polyfills__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(dom_node_polyfills__WEBPACK_IMPORTED_MODULE_2__);
...
/* harmony import */ var _modules_calc__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ./modules/calc */ "./js/modules/calc.js");
/* harmony import */ var _modules_forms__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./modules/forms */ "./js/modules/forms.js");
/* harmony import */ var _modules_slider__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ./modules/slider */ "./js/modules/slider.js");
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  }, 7000); // Функция предварительной загрузки изображения спиннера
  ...
  preloadImg(['img/form/spinner.svg']); // const tabs = require('./modules/tabs'),
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__.default)('.timer', '2021-01-30');
  (0,_modules_cards__WEBPACK_IMPORTED_MODULE_8__.default)();
  (0,_modules_calc__WEBPACK_IMPORTED_MODULE_9__.default)();
  (0,_modules_forms__WEBPACK_IMPORTED_MODULE_10__.default)(modalTimerId);
